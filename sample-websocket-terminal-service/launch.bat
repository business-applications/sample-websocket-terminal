...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "sample-websocket-terminal-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name sample-websocket-terminal-service apps/sample-websocket-terminal-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app sample-websocket-terminal-service:1.0-SNAPSHOT
		call oc expose svc/sample-websocket-terminal-service
	) else (
