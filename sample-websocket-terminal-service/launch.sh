...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-websocket-terminal-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-websocket-terminal-service apps/sample-websocket-terminal-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-websocket-terminal-service:1.0-SNAPSHOT
    oc expose svc/sample-websocket-terminal-service
else

	echo "Launching the application locally..."
	pattern="sample-websocket-terminal-service"
	files=( $pattern )
	cd ${files[0]}
